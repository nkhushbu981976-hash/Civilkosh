...
function initConcrete(){
 const fields=['length','width','depth'].map(id=>document.getElementById(id));
 const units={m:1,ft:0.3048,in:0.0254};
 const selects=['lengthUnit','widthUnit','depthUnit'].map(id=>document.getElementById(id));
 const m3=document.getElementById('concreteM3'),ft3=document.getElementById('concreteFt3'),error=document.getElementById('concreteError');
 if(fields.some(x=>!x)||selects.some(x=>!x)||!m3||!ft3||!error)return;
 const format=n=>new Intl.NumberFormat('en-US',{maximumSignificantDigits:12}).format(n);
 function calculate(){
  const values=fields.map(x=>Number(x.value));
  if(fields.some(x=>x.value.trim()==='')||values.some(x=>!Number.isFinite(x)||x<=0)){m3.textContent='—';ft3.textContent='—';error.hidden=false;return}
  error.hidden=true;
  const volumeM3=values.reduce((total,n,i)=>total*n*units[selects[i].value],1);
  m3.textContent=format(volumeM3);ft3.textContent=format(volumeM3*35.3146667215);
 }
 fields.forEach(x=>x.addEventListener('input',()=>{if(x.value!=='')error.hidden=true}));
 selects.forEach(x=>x.addEventListener('change',()=>{if(fields.every(f=>f.value.trim()!==''))calculate()}));
 document.getElementById('calculateConcrete')?.addEventListener('click',calculate);
 document.getElementById('resetConcrete')?.addEventListener('click',()=>{fields.forEach(x=>x.value='');selects.forEach(x=>x.value='m');m3.textContent='—';ft3.textContent='—';error.hidden=true;fields[0].focus()});
}

function initHome(){
 const grid=document.querySelector('#toolGrid');if(!grid)return;
...
document.addEventListener('DOMContentLoaded',()=>{if(isConverter)initConverter();else if(isConcrete)initConcrete();else initHome()});
