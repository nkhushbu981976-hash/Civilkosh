...
const isConverter=path.endsWith('land-unit-converter.html');
...
if(isConverter){
 ...
 const format=n=>{if(!Number.isFinite(n))return '—';return new Intl.NumberFormat('en-US',{maximumSignificantDigits:12}).format(n)};
 function convert(){const n=Number(value.value);if(value.value.trim()===''||!Number.isFinite(n)||n<0){result.textContent='—';error.hidden=false;return}error.hidden=true;const converted=n*units[from.value].sqm/units[to.value].sqm;result.textContent=format(converted);resultUnit.textContent=`${units[to.value].name} (${units[to.value].symbol})`;}
 value.addEventListener('input',convert);from.addEventListener('change',convert);to.addEventListener('change',convert);
 document.querySelector('#swapButton').addEventListener('click',()=>{const old=from.value;from.value=to.value;to.value=old;convert()});
 ...
 const land=document.querySelector('.tool-card');
 if(land&&tools[0].title==='Nepal Land Unit Converter')land.style.cursor='pointer';
 document.querySelectorAll('.tool-card').forEach(card=>{if(card.querySelector('h3')?.textContent===tools[0].title)card.addEventListener('click',()=>location.href='land-unit-converter.html')});
}
